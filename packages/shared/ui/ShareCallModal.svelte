<script lang="ts">
  ...
  type ShareSummary = {
    ...
    view_count: number;
    // #454 — last time anyone opened this link. Optional for backward
    // compat with backends that don't yet emit the field.
  ...
  // #363 — Section presets. Two quick-pick buttons above the toggle
  // fieldset so creators don't have to reason through each checkbox.
  // "Public summary only" = just the AI summary; transcript, audio,
  // actions and notes all off (safe for external recipients who don't
  // need the verbatim record). "Internal full" = the v0.7.2 default —
  // every primary section on, notes + download still off (opt-in).
  type Preset = "summary-only" | "internal-full";
  ...
  // Detect which preset (if any) the current toggle state matches.
  let activePreset = $derived.by<Preset | null>(() => {
    if (
      !included.manual_notes &&
      included.summary &&
      !included.action_items &&
      !included.transcript &&
      !included.audio &&
      !included.allow_download
    ) return "summary-only";
    if (
      !included.manual_notes &&
      included.summary &&
...
>
  ...
  >
    ...
    <div class="rn-body">
      ...
      <!-- #363: preset quick-picks. Two common shapes so the creator
           doesn't have to reason through every toggle. The active
           preset button is highlighted; any manual toggle deviation
           clears the highlight so the state is always honest. -->
      <div class="preset-row" aria-label="Section presets">
        <span class="preset-label">Presets:</span>
        <button
          type="button"
          class="preset-btn"
          class:active={activePreset === "summary-only"}
          onclick={() => applyPreset("summary-only")}
          disabled={creating}
          title="Summary only — safe for external recipients"
        >
          Public summary only
        </button>
        <button
          type="button"
          class="preset-btn"
          class:active={activePreset === "internal-full"}
          onclick={() => applyPreset("internal-full")}
          disabled={creating}
          title="Summary + actions + transcript + audio — for internal teammates"
        >
          Internal full
        </button>
      </div>
            ...
            {#each shares as s (s.id)}
              ...
              <li class="share-row" class:revoked={s.status !== "active"}>
                ...
                <div class="share-row-meta">
                  <!-- #454 — view count + last-accessed surfaced
                       prominently. last_accessed_at is optional (older
                       backends that don't emit it fall back gracefully). -->
                  <div class="share-row-stats">
                    <span class="muted">Created {fmtDate(s.created_at)}</span>
                    <span class="share-stat-sep" aria-hidden="true">·</span>
                    <span
                      class="share-view-count"
                      class:has-views={s.view_count > 0}
                      title="{s.view_count} {s.view_count === 1 ? 'view' : 'views'}"
                    >
                      {s.view_count}
                    ...
                    {#if s.last_accessed_at}
                      <span class="share-stat-sep" aria-hidden="true">·</span>
                      <span class="muted" title="Last accessed">
                        Last: {fmtDate(s.last_accessed_at)}
                      </span>
                    {:else if s.view_count === 0}
                      <span class="share-stat-sep" aria-hidden="true">·</span>
                      <span class="muted">Not yet opened</span>
                    {/if}
                  </div>
                  {#if s.status === "active"}
                    <button
...
<style>
  ...
  /* #454 — view count + last-accessed row */
  .share-row-stats {
    display: flex;
    align-items: center;
    gap: 0.35rem;
  ...
  /* View count is muted until the link has actually been seen. */
  .share-view-count {
    color: var(--bone-4);
  }
  .share-view-count.has-views {
    color: var(--bone-2);
    font-weight: 500;
  }
  .muted {
  ...
  /* #363 — Preset quick-pick row above the toggle fieldset. */
  .preset-row {
    display: flex;
    align-items: center;
    gap: 0.45rem;
