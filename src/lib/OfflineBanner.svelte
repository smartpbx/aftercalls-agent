<script lang="ts">
  ...
  // the shared `onlineStatus` store flips to offline, surface a small
  ...
  //
  // Mirrored byte-identical between portal/src/lib/OfflineBanner.svelte
  // and agent/src/lib/OfflineBanner.svelte. Both surfaces have a
  // sibling onlineStatus store at the same import path; the store is
  // surface-specific (portal hits /health via fetch, agent rides a
  // Tauri command) but its `online` property has the same shape, so
  // this component never branches.

  import { onlineStatus } from "$lib/stores/onlineStatus.svelte";

  let online = $derived(onlineStatus.online);
</script>
